...
#### Backblaze B2 (DVC - Data Storage)

**You'll receive these from the project owner:**

```bash
AWS_ACCESS_KEY_ID=<provided_by_owner>
AWS_SECRET_ACCESS_KEY=<provided_by_owner>
```

> **Note:** The owner will share these credentials securely. Do NOT create your own Backblaze account.
...
DVC needs your Backblaze B2 credentials configured locally (won't be committed to git):
...
Download all project data from Backblaze B2:
...
- Make sure your `.env` file has `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`
...
**"dvc pull" fails**
- Verify Backblaze B2 credentials in `.env` are correct
